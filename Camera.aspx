<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Camera.aspx.cs" Inherits="_Default" %>
  ...
  <!--             库文件调用                     -->
       ...
       <%-- <script charset="gb2312" language="javascript" src="lib/ShaderSkin.js" type="text/javascript"></script>--%>
         ...
         <script charset="gb2312" language="javascript" src="lib/hilbert3D.js" type="text/javascript"></script>
          ...
          <!--           3djs库文件调用                     -->
        <script charset="gb2312" language="javascript" src="3Djs/VariableClass_Standard_CameraArray.js" type="text/javascript"></script>
    ...
    function InitWebgl() {
        ...
        for (var i = 0; i < m_VarGlobal.m_XMLWall.length; i++) {
            ...
            for (var x = 0; x < AMOUNT; x++) {
                ...
                if (x == 0) {
                    subcamera.bounds = new THREE.Vector4(0.0, 0.0, 0.5, 1.0);
                } else if (x == 1) {
                    subcamera.bounds = new THREE.Vector4(0.75, 0.75, 0.25, 0.25);
                }
                // subcamera.position.x = (x / AMOUNT) - 0.5;
                // subcamera.position.y = 0.5 - (y / AMOUNT);
                subcamera.position.z = 100.0;
        ...
        m_VarGlobal.m_Cameras = new THREE.ArrayCamera(cameras);
     
        
      //  m_VarGlobal.m_Cameras.position.z = 500;
      //  alert(m_VarGlobal.m_Cameras.position.z)
        m_VarGlobal.m_BaseSence = new BaseScene();
        m_VarGlobal.m_Animation = new AnimationClass();
      

        m_VarGlobal.m_BaseSence.m_Scene.add(new THREE.AmbientLight(0x222244));

        var light = new THREE.DirectionalLight();
        light.position.set(0.5, 0.5, 1);
        light.castShadow = true;
        light.shadow.camera.zoom = 4; // tighter shadow map
        m_VarGlobal.m_BaseSence.m_Scene.add(light);
...
//        Obj_Add(mesh)
    ...
    //    var m_obj = SeachObj(m_VarGlobal.m_BaseSence.m_dis, "fan7");
      
       // alert(m_obj.material.normalMap);
        ///////////////////关键帧动画
