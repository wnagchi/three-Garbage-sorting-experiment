...
ToothObj.prototype.CreateFaces = function (material, transparency) {//创建面
    ...
    for (var i = 0; i < m_colNum * m_rowNum - 1; i++) {
        ...
        if (i % m_colNum < 63) {
            ...
            var face = new THREE.Face3(i, i - 63, i + 2);
            geometry.faces.push(face);

            // alert(i + this.m_IMax)
            var face = new THREE.Face3(i, i + 2, i + this.m_IMax + 1);
            geometry.faces.push(face);
        }

        //var face = new THREE.Face3(i, i + 1, i + this.m_IMax + 2);
        //geometry.faces.push(face);
        //var face = new THREE.Face3(i, i + this.m_IMax + 2, i + this.m_IMax + 1);
        //geometry.faces.push(face);

        //var m_rowy = Math.floor(i / (this.m_IMax + 1));
        //        geometry.faceVertexUvs[0].push([
        //            new THREE.Vector2(1 - m_rowy / m_rowNum, 1 - i % m_colNum / m_colNum),
        //            new THREE.Vector2(1 - m_rowy / m_rowNum, 1 - (i % m_colNum + 1) / m_colNum),
        //            new THREE.Vector2(1 - (m_rowy + 1) / m_rowNum, 1 - (i % m_colNum + 1) / m_colNum),
        //            new THREE.Vector2(1 - (m_rowy + 1) / m_rowNum, 1 - i % m_colNum / m_colNum)
        //        ]);
        //geometry.faces.push(face);
    }
    //geometry.mergeVertices();
    //geometry.computeCentroids();
    // alert(geometry.faceVertexUvs[0].length)
    ...
    //var m_obj = new THREE.Object3D();
    //m_obj.add(m_Obj_dis);
    //m_VarGlobal.GoodsList.push(m_obj);
    this.m_face = m_Obj_dis;
    this.m_face.visible = true;
    this.m_face.name = "face_test";

    Obj_Add(m_Obj_dis); //添加牙面
   // alert(this.m_face.name);
    return m_Obj_dis;
}

ToothObj.prototype.CreateFaces_1 = function (material, transparency) {//创建面
    var geometry = new THREE.Geometry();
    geometry.vertices = this.arr_geometry.vertices;
    var m_rowNum = geometry.vertices.length / (this.m_IMax + 1);
    var m_colNum = (this.m_IMax + 1);
    ...
    loader.load('fonts/helvetiker_bold.typeface.js', function (font) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (i >= rownum * (m_scope.m_IMax + 1) && i < (rownum + 1) * (m_scope.m_IMax + 1) && i % step == 0) {
                var m_t = Create3dText("" + i % m_num, 2, font);
